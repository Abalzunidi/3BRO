...
export function Admin() {
  ...
  const handleSave = () => {
    ...
    if (!username && pin.length !== 4) {
      toast('Add a username or a 4-digit code', 'error')
      return
    }
    if (pin && pin.length !== 4) {
      toast('Code must be 4 digits', 'error')
      return
    }
    if (username) {
      const takenUser = members.some((m) => {
        if (m.id === editing?.id) return false
        return normalizeUsername(m.username || '') === username
      })
      if (takenUser) {
    ...
    if (pin.length === 4) {
      const takenPin = members.some((m) => m.pin === pin && m.id !== editing?.id)
      if (takenPin) {
        toast('This code is already used', 'error')
        return
      }
    }
    const payload = {
    ...
    <div className="space-y-6 pb-20">
      ...
      {members.length === 0 ? (
        <EmptyState
          ...
          description="Add people and give each one a username, a 4-digit code, or both."
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-muted-foreground mt-1 space-x-2">
                  {member.username && (
                    <>
                      Username: <span className="font-medium text-foreground" dir="auto">{member.username}</span>
                    </>
                  )}
                  {member.username && member.pin?.length === 4 && ' · '}
                  {member.pin?.length === 4 && (
                    <>
                      Code: <span className="font-display font-semibold tracking-widest text-foreground">{member.pin}</span>
                    </>
                  )}
                </p>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <Label>Code (4 digits, optional)</Label>
              <Input
                inputMode="numeric"
                value={form.pin}
